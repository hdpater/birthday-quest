...
function spawnMonster(x, y) {
  ...
  // Pick monster whose level is within ±4
  const pool = M_LEVELS.map((l,i)=>({i,l})).filter(({l})=>Math.abs(l-targetLvl)<=4);
...
function MagicShopDialogue({building,heroState,setHeroState,hasMap,setHasMap,onDismiss}){
  ...
  const buy=(form,type)=>{
    ...
    const item={id:`magic_${Date.now()}_${Math.random()}`,name:`${type[0].toUpperCase()+type.slice(1)} ${form[0].toUpperCase()+form.slice(1)}`,type:"magic",form,magicType:type,cost:price,color:MAGIC_COLOR[type]};
...
export default function Game(){
  ...
  const walkPath=useCallback(()=>{
    ...
    // Merchant? (0.2%)
    if(Math.random()<0.01){stopAt(nx,ny);addLog("A wandering merchant appears!");setMerchantStock(generateMerchantStock());setModal({type:"merchant"});return;}
    // Monster? (1%)
    if(Math.random()<0.01){
      const mon=spawnMonster(nx,ny);
      stopAt(nx,ny);addLog(`A ${mon.name} (Lv${mon.level}) blocks your path!`);
      setModal({type:"combat",data:mon});return;
    }
